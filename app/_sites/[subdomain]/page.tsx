...
export default function SubdomainStorePage() {
  ...
  useEffect(() => {
    if (!subdomain) return;
    fetch(`/api/vendors/subdomain/${subdomain}`)
      .then(r => r.json())
      .then(json => {
      ...
      return (
        <div className="min-h-screen bg-white dark:bg-charcoal-900 flex items-center justify-center">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-charcoal-900 dark:text-white mb-2">Store Not Found</h1>
            <p className="text-charcoal-600 dark:text-cool-gray-400">We couldn't find a store at this subdomain.</p>
          </div>
        </div>
      );
    }
