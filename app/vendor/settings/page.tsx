      ...
      <div>
        ...
        >
          {preview ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-charcoal-400 dark:text-cool-gray-500 bg-cool-gray-50 dark:bg-charcoal-700">
              {uploading ? (
                ...
                <>
                  ...
                  <span className="text-sm">Click or drag &amp; drop banner</span>
  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-5">
        <div
          onClick={() => inputRef.current?.click()}
          className="relative w-28 h-28 rounded-full overflow-hidden cursor-pointer border-2 border-dashed border-cool-gray-300 dark:border-charcoal-600 hover:border-gold-400 transition-all group flex-shrink-0 bg-cool-gray-50 dark:bg-charcoal-700"
        >
          {preview ? (
            <>
              <Image src={preview} alt={label} fill className="object-cover" unoptimized />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all flex items-center justify-center rounded-full">
        ...
        <div className="text-sm text-charcoal-500 dark:text-cool-gray-400 pt-1">
          <p className="font-medium text-charcoal-700 dark:text-cool-gray-300 mb-1">Click the circle to upload</p>
          <p>{hint}</p>
          <button
            type="button"
            onClick={() => inputRef.current?.click()}
...
export default function VendorSettingsPage() {
      ...
      .then(d => {
        if (d.success && d.data?.user) {
          ...
          setStoreData(prev => ({
            ...
            storeName: u2.storeName || prev.storeName,
            bio:       u2.bio       || prev.bio,
  ...
  const handleSaveStore = async () => {
    ...
    try {
      ...
      showMsg('success', '✅ Store profile saved — changes will appear live on your vendor page!');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-cool-gray-50 dark:bg-charcoal-950">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              <Link href="/vendor/dashboard" className="text-charcoal-400 hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
                </svg>
              </Link>
              <div>
                <h1 className="text-lg font-bold">Vendor Settings</h1>
                <p className="text-xs text-charcoal-400">Manage your store profile</p>
              </div>
            </div>
            <Link href="/vendor/dashboard" className="text-sm text-gold-400 hover:text-gold-300 transition-colors">
              ← Back to Dashboard
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-charcoal-900 dark:text-white">Store Settings</h2>
          <p className="text-charcoal-500 dark:text-cool-gray-400 mt-1">
            Customize how your store looks to shoppers — upload your profile photo and banner to make a great impression.
          </p>
        </div>
        ...
        {!storeData.avatar && !storeData.banner && (
          <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-amber-700 dark:text-amber-400 text-xs mt-0.5">Upload a profile photo and store banner to build trust with buyers and increase sales.</p>
            </div>
          </div>
        )}
          ...
          <div className="lg:col-span-3 space-y-5">
              ...
              <div className="bg-white dark:bg-charcoal-800 rounded-2xl shadow-sm border border-cool-gray-100 dark:border-charcoal-700 p-6 space-y-8">
                <div>
                  ...
                  <p className="text-sm text-charcoal-500 dark:text-cool-gray-400">
                    These photos appear publicly on your vendor store page and in the vendors directory.
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-charcoal-700 dark:text-cool-gray-300 mb-2">
                    Store Name <span className="text-xs font-normal text-charcoal-400 dark:text-cool-gray-500">(shown publicly on your store page)</span>
                  </label>
                  <input
                    type="text"
                    value={storeData.storeName}
                    onChange={e => setStoreData(prev => ({ ...prev, storeName: e.target.value }))}
                    placeholder={`${storeData.firstName} ${storeData.lastName}`.trim() || 'e.g. Luxury Fashion Co.'}
                    className="w-full px-4 py-3 border border-cool-gray-200 dark:border-charcoal-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent bg-white dark:bg-charcoal-700 text-charcoal-900 dark:text-white placeholder-charcoal-400 dark:placeholder-cool-gray-500 text-sm"
                  />
                  <p className="mt-1 text-xs text-charcoal-400 dark:text-cool-gray-500">Leave blank to use your first + last name as the store name.</p>
                </div>

                <UploadZone
                  label="Profile Photo / Store Logo"
                  hint="Recommended: 400×400px square image — PNG, JPG or WebP"
                ...
                <UploadZone
                  ...
                  hint="Recommended: 1500×500px wide image — shown at the top of your store page"
                ...
                <div>
                  ...
                  <div className="rounded-2xl overflow-hidden border border-cool-gray-200 dark:border-charcoal-700 shadow-md max-w-xs">
                    <div className="relative h-24 bg-gradient-to-br from-charcoal-800 to-charcoal-900">
                      {storeData.banner && (
                        <Image src={storeData.banner} alt="Banner" fill className="object-cover opacity-70" unoptimized />
                      )}
                      {storeData.avatar ? (
                        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-16 h-16 rounded-full border-4 border-white dark:border-charcoal-800 overflow-hidden shadow-lg bg-white">
                          <Image src={storeData.avatar} alt="Avatar" fill className="object-cover" unoptimized />
                        </div>
                      ) : (
                        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-16 h-16 rounded-full border-4 border-white dark:border-charcoal-800 bg-gradient-to-br from-gold-600 to-gold-800 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                          {storeData.firstName.charAt(0) || 'V'}
                        </div>
                      )}
                    </div>
                    <div className="pt-10 pb-4 px-4 bg-white dark:bg-charcoal-800 text-center">
                      <p className="font-bold text-sm text-charcoal-900 dark:text-white">{storeData.firstName || 'Your Store'} {storeData.lastName}</p>
                      <p className="text-xs text-charcoal-500 dark:text-cool-gray-400 mt-1 line-clamp-2">{storeData.bio || 'Store description…'}</p>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'profile' && (
              <div className="bg-white dark:bg-charcoal-800 rounded-2xl shadow-sm border border-cool-gray-100 dark:border-charcoal-700 p-6 space-y-5">
                <div>
                  <h3 className="text-lg font-bold text-charcoal-900 dark:text-white mb-1">Store Information</h3>
                  <p className="text-sm text-charcoal-500 dark:text-cool-gray-400">Update your store name and description.</p>
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-charcoal-700 dark:text-cool-gray-300 mb-2">First Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-charcoal-700 dark:text-cool-gray-300 mb-2">Last Name</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-semibold text-charcoal-700 dark:text-cool-gray-300 mb-2">Store Bio</label>
                  <textarea
                    value={storeData.bio}
                    onChange={e => setStoreData(prev => ({ ...prev, bio: e.target.value }))}
                    rows={4}
            ...
            {activeTab === 'password' && (
              <div className="bg-white dark:bg-charcoal-800 rounded-2xl shadow-sm border border-cool-gray-100 dark:border-charcoal-700 p-6 space-y-5">
                <div>
                  ...
                  <p className="text-sm text-charcoal-500 dark:text-cool-gray-400">Keep your vendor account secure.</p>
                </div>
